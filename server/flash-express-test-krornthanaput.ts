/**
 * ไฟล์ทดสอบการเชื่อมต่อกับ Flash Express API ด้วยข้อมูลของคุณกรธนภัทร
 */
...
/**
 * สร้างลายเซ็นตามมาตรฐานของ Flash Express ใช้เฉพาะสำหรับ HTTP Header
 */
function generateFlashSignature(params: Record<string, any>, apiKey: string): string {
  try {
    console.log('⚙️ เริ่มคำนวณลายเซ็น Flash Express API (เฉพาะ header)...');
...
/**
 * ทดลองใช้ข้อมูลของคุณกรธนภัทร
 */
async function testWithKrornthanaput() {
  try {
    console.log('🧪 เริ่มการทดสอบ Flash Express API ด้วยข้อมูลของคุณกรธนภัทร...');
    ...
    const orderData = {
      ...
      srcPostalCode: '10230',
      srcDetailAddress: 'ลาดปลาเค้า 24 แยก 8 จรเข้บัว ลาดพร้าว กรุงเทพ 10230',
      
      // ข้อมูลผู้รับ (ใช้ข้อมูลเดิมเป็นตัวอย่าง)
      dstName: 'วาสนา วงศ์มาลา',
      dstPhone: '0922573604',
      dstProvinceName: 'น่าน',
      dstCityName: 'เวียงสา',
      dstDistrictName: 'กลางเวียง',
      dstPostalCode: '55110',
      dstDetailAddress: '2หมู่12 บ.ร้องเย็น ต.กลางเวียง อ.เวียงสา จ.น่าน 55110',
      ...
      remark: 'ทดสอบการส่งพัสดุด้วยข้อมูลของคุณกรธนภัทร'
    };
    
    // 3. คำนวณลายเซ็น
    const signature = generateFlashSignature(orderData, FLASH_EXPRESS_API_KEY as string);
    
    // 4. ข้อมูลที่จะส่ง - ทดลองหลายวิธี
    
    // วิธีที่ 1: รวมลายเซ็นในข้อมูล
    const requestData1 = {
      ...orderData,
      sign: signature
    };
    
    // วิธีที่ 2: ไม่รวมลายเซ็นในข้อมูล เพิ่มทางทาง Header
    const requestData2 = { ...orderData };
    
    // เลือกใช้วิธีที่ 2
    const requestData = requestData2;
    ...
    // 6. ตั้งค่า headers - ส่งลายเซ็นผ่าน header
    const headers = {
    ...
    console.log('Encoded payload ที่ส่งไป:', formData.toString().substring(0, 200) + '...');
...
// รันการทดสอบ
testWithKrornthanaput().then(result => {
  console.log('\n🏁 ผลการทดสอบ:', result);
}).catch(error => {
