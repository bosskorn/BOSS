/**
 * ไฟล์ทดสอบการเชื่อมต่อกับ Flash Express API โดยส่งลายเซ็นผ่าน HTTP Header เท่านั้น
 */
...
/**
 * ทดลองใช้วิธีส่งลายเซ็นผ่าน HTTP Header เท่านั้น (ไม่ใส่ใน body)
 */
async function testWithHeaderOnly() {
  try {
    console.log('🧪 เริ่มการทดสอบ Flash Express API ด้วยวิธีส่งลายเซ็นผ่าน HTTP Header...');
    ...
    const orderData = {
      ...
      remark: 'ทดสอบการส่งพัสดุด้วยวิธีส่งลายเซ็นผ่าน header'
    };
    
    // 3. คำนวณลายเซ็น (ใช้ข้อมูลเดียวกับที่ส่งในคำขอ)
    const signature = generateFlashSignature(orderData, FLASH_EXPRESS_API_KEY as string);
    
    // 4. สร้างข้อมูลคำขอ (ไม่ใส่ลายเซ็นใน body)
    ...
    // 6. ตั้งค่า headers - ส่งลายเซ็นผ่าน header เท่านั้น
    const headers = {
...
// รันการทดสอบ
testWithHeaderOnly().then(result => {
  console.log('\n🏁 ผลการทดสอบ:', result);
}).catch(error => {
