...
const OrderList: React.FC = () => {
  ...
  const [multipleTrackingDialogOpen, setMultipleTrackingDialogOpen] =
    useState(false);
  ...
  const [orderToCreateTracking, setOrderToCreateTracking] = useState<
    number | null
  >(null);
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (data.success) {
        console.log(
          "ข้อมูลออเดอร์อยู่ใน orders field:",
        ...
        const ordersWithCarrier = (data.orders || []).map((order: any) => {
          ...
          if (order.trackingNumber) {
            ...
            if (order.trackingNumber.startsWith("FLE")) {
              console.log(
                `ตรวจสอบ order #${order.id} ${order.orderNumber}: ${order.trackingNumber}, shippingMethod: ${order.shippingMethod || "ไม่ระบุ"}`,
              );
              console.log(
                `✓ Matched Order #${order.id}: ${order.trackingNumber} by prefix FLE`,
              );
              carrier = "flash-express";
            } else if (order.trackingNumber.startsWith("JT")) {
              console.log(
                `✓ Matched Order #${order.id}: ${order.trackingNumber} by prefix JT`,
              );
              carrier = "jt-express";
            } else if (order.trackingNumber.startsWith("XB")) {
              console.log(
                `✓ Matched Order #${order.id}: ${order.trackingNumber} by prefix XB`,
              );
        ...
        const ordersWithTracking =
          await fetchTrackingStatusForOrders(ordersWithCarrier);
  ...
  const filterFlashExpressOrders = () => {
    ...
    console.log(
      `จำนวนออเดอร์ของ Flash Express หลังกรอง: ${flashExpressOrders} รายการ`,
    );
    return filtered;
  };
  ...
  const filterOrders = () => {
    ...
    if (activeTab === "pending") {
      ...
      result = result.filter(
        (order) => !order.tracking_number && !order.trackingNumber,
      );
      ...
      result = result.filter(
        (order) =>
          ...
          (order.trackingStatus && order.trackingStatus.includes("ยกเลิก")),
      );
      ...
      result = result.filter(
        (order) =>
          ...
          (order.tracking_number &&
            (order.tracking_number.startsWith("FLE") ||
              order.tracking_number.startsWith("TH"))),
      );
    ...
    if (shippingMethodFilter !== "all") {
      if (shippingMethodFilter === "flash-express") {
        result = result.filter((order) =>
          order.trackingNumber?.startsWith("FLE"),
        );
  ...
  const fetchTrackingStatusForOrders = async (orders: Order[]) => {
    ...
    console.log(
      `กำลังดึงสถานะการติดตามสำหรับ ${ordersWithValidTracking.length} ออเดอร์ที่มีเลขพัสดุ`,
    );
      ...
      try {
        ...
        if (data.success && data.trackingData) {
          ...
          // กรณีมีข้อมูลแบบเก่า (history)
            ...
            data.trackingData.history &&
            data.trackingData.history.length > 0
          ) {
        ...
        console.error(
          `เกิดข้อผิดพลาดในการดึงข้อมูลติดตามพัสดุเลข ${trackingNo}:`,
          error,
        );
    ...
    return orders.map((originalOrder) => {
      ...
      const orderWithTracking = ordersWithTrackingStatus.find(
        (o) => o.id === originalOrder.id,
      );
  ...
  return (
    <Layout>
              ...
              <Button
                ...
                onClick={() => {
                  if (selectedOrders.length > 0) {
                    ...
                    window.open(
                      `/print-multiple-labels-fixed?orders=${orderIds}`,
                      "_blank",
                    );
                  } else {
                    toast({
                    ...
                    <span>
                      รอดำเนินการ (
                      {
                        filteredOrders.filter(
                          (o) => !o.tracking_number && !o.trackingNumber,
                        ).length
                      }
                      )
                    </span>
              ...
              <Table>
                <TableHeader className="bg-white sticky top-0">
                  <TableRow>
                    ...
                    <TableHead
                      className="py-3 font-medium text-gray-500 cursor-pointer"
                    ...
                    <TableHead className="py-3 font-medium text-gray-500">
                      เลขพัสดุ
                    </TableHead>
                    <TableHead className="py-3 font-medium text-gray-500">
                      การดำเนินการ
                      ...
                      .map((order) => (
                        ...
                        >
                          ...
                          <TableCell>
                            ...
                            (order.tracking_number &&
                                order.tracking_number.startsWith("แบบ")) ||
                                ...
                                onClick={() => {
                                  ...
                                  setTimeout(() => {
                                    ...
                                    fetch(`/api/tracking/status/${trackingNo}`)
                                      ...
                                      .catch((error) => {
                                        console.error(
                                          "เกิดข้อผิดพลาดในการดึงข้อมูลติดตามพัสดุ:",
                                          error,
                                        );
                                        toast({
                                          title: "เกิดข้อผิดพลาด",
                                          description:
                                            "ไม่สามารถดึงข้อมูลติดตามพัสดุได้",
                                          variant: "destructive",
                                        });
                                      })
                                      .finally(() => {
                            ...
                            <div className="flex space-x-1">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  if (
                                    confirm(
                                      `ต้องการลบออเดอร์ ${order.orderNumber} ใช่หรือไม่?`,
                                    )
                                  ) {
              ...
              {filteredOrders.length > itemsPerPage && (
                <div className="flex flex-col sm:flex-row justify-between items-center py-4 px-6 bg-white border-t border-gray-200">
                  <div className="text-sm text-gray-600 mb-4 sm:mb-0">
                    ...
                    จากทั้งหมด {filteredOrders.length} รายการ
                  </div>
          ...
          <div className="py-4">
            {isLoadingTracking ? (
              ...
              <div className="space-y-4">
                ...
                (trackingData.routes && trackingData.routes.length > 0) ? (
                  <div className="rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    <ul className="divide-y divide-gray-200">
                      ...
                      {trackingData.routes &&
                        trackingData.routes.map((item: any, index: number) => (
                            ...
                            <div className="flex items-start">
                              ...
                              <div className="ml-3">
                                ...
                                <div className="mt-1 text-sm text-gray-500 space-y-1">
                                  <p>
                                    {item.operationAddress || "ไม่ระบุสถานที่"}
                                  </p>
                        ...
                        trackingData.history.map((item: any, index: number) => (
                          <li key={`history-${index}`} className="px-4 py-3">
                            <div className="flex items-start">
                              ...
                              <div className="ml-3">
                                <p className="text-sm font-medium text-gray-900">
                                  {item.status || "อัปเดตสถานะ"}
                                </p>
                                <div className="mt-1 text-sm text-gray-500 space-y-1">
                                  <p>{item.location || "ไม่ระบุสถานที่"}</p>
                                  <p className="text-xs text-gray-400">
                                    {item.datetime || "ไม่ระบุเวลา"}
                                  </p>
                                </div>
                              </div>
                            </div>
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-500">
                  ขนาด 100mm x 150mm สำหรับทุกการขนส่ง
                </p>
              </div>
              <div
                className={`h-5 w-5 rounded-full border-2 p-0.5 ${selectedLabelType === "flash" ? "border-blue-600" : "border-gray-300"}`}
              >
            ...
            <Button
              onClick={() => {
                ...
                if (isPrintingMultiple) {
                  // กรณีพิมพ์หลายรายการ
                  const selectedOrdersData = filteredOrders.filter(
                    (order: Order) => selectedOrders.includes(order.id),
                  );
                  // กรองเฉพาะออเดอร์ที่มีเลขพัสดุ
                  const ordersToPrint = selectedOrdersData.filter(
                    (order: Order) =>
                      order.tracking_number || order.trackingNumber,
                  );
                  ...
                  if (ordersToPrint.length === 0) {
                    toast({
                      ...
                      description:
                        "ออเดอร์ที่เลือกไม่มีเลขพัสดุ กรุณาสร้างเลขพัสดุก่อนพิมพ์ลาเบล",
                      variant: "destructive",
                    });
                    return;
                  }
      ...
      <Dialog
        open={multipleTrackingDialogOpen}
        onOpenChange={setMultipleTrackingDialogOpen}
      >
              ...
              <div className="space-y-4">
                {selectedOrders.length > 0 ? (
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setTimeout(() => {
                                  ...
                                  fetch(
                                    `/api/tracking/status/${order.tracking_number || order.trackingNumber}`,
                                  )
                                    .then((response) => response.json())
                                    .then((data) => {
                  ...
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      กรุณาเลือกรายการที่ต้องการดูข้อมูลการติดตาม
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="py-4 space-y-4">
            ...
            <div className="rounded-lg border border-gray-200 p-4 text-sm">
              <p className="font-medium text-gray-700 mb-2">
                ข้อมูลการสร้างเลขพัสดุ
              </p>
              <p className="text-gray-600 mb-1">
                - คุณกำลังสร้างเลขพัสดุสำหรับออเดอร์ #{orderToCreateTracking}
              </p>
              <p className="text-gray-600 mb-1">
                - เลขพัสดุที่สร้างจะถูกบันทึกและไม่สามารถเปลี่ยนแปลงได้
              </p>
              <p className="text-gray-600">
                - คุณสามารถพิมพ์ลาเบลได้หลังจากสร้างเลขพัสดุเรียบร้อยแล้ว
              </p>
            </div>
          </div>
          <DialogFooter>
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const response = await fetch(
                    `/api/shipping-methods/${selectedShippingMethod}/create-tracking`,
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
                  ...
                  if (data.success) {
                    ...
                    if (confirm("ต้องการพิมพ์ลาเบลหรือไม่?")) {
                      ...
                      const order = await fetch(
                        `/api/orders/${orderToCreateTracking}`,
                        {
                          method: "GET",
                          headers: {
                            "Content-Type": "application/json",
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              คุณกำลังจะลบรายการที่เลือกทั้งหมด {selectedOrders.length} รายการ
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    ...
                    <li>
                      หากรายการมีการเชื่อมโยงกับข้อมูลอื่น อาจไม่สามารถลบได้
                    </li>
                ...
                for (const orderId of selectedOrders) {
                  try {
                    ...
                    if (data.success) {
                      ...
                      errorCount++;
                      console.error(
                        `ไม่สามารถลบออเดอร์ #${orderId}: ${data.message}`,
                      );
                    }
                  } catch (error) {
                    errorCount++;
                    console.error(
                      `ข้อผิดพลาดในการลบออเดอร์ #${orderId}:`,
                      error,
                    );
                  }
                }
